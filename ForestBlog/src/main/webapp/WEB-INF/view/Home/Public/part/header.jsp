            ...
            <div class="user-login">
                <c:choose>
                    <c:when test="${sessionScope.user==null}">
                        <a href="/ForestBlog/admin">登录</a>
                    </c:when>
                    <c:otherwise>
                        <a href="/ForestBlog/admin">进入后台</a>
                    </c:otherwise>
                </c:choose>
    ...
    <div id="menu-box">
        <div id="top-menu">
            ...
            <div class="logo-site"><h1 class="site-title">
                <a href="/ForestBlog/" title="${options.optionSiteTitle}">${options.optionSiteTitle}</a>
            </h1>
                        ...
                        <ul id="menu-pcmenu" class="down-menu nav-menu sf-js-enabled sf-arrows">
                            ...
                            <c:forEach items="${allCategoryList}" var="category">
                                <c:if test="${category.categoryPid==0}">
                                    <li>
                                        <a href="/ForestBlog/category/${category.categoryId}">
                                        ...
                                        <ul class="sub-menu">
                                            <c:forEach items="${allCategoryList}" var="cate">
                                                <c:if test="${cate.categoryPid==category.categoryId}">
                                                    <li>
                                                        <a href="/ForestBlog/category/${cate.categoryId}" target="_blank">${cate.categoryName}</a>
                                                    </li>
                                                </c:if>
                                            </c:forEach>
                                        </ul>
